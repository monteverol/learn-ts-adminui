...
const parseYYYYMM = (ym) => ym ? new Date(`${ym}-01T00:00:00.000Z`) : null;
const toStatus = (s) => s === 'ARCHIVED' ? UserStatus.ARCHIVED : UserStatus.ACTIVE;
const toJobCategory = (c) => {
    if (!c)
        return null;
    if (c === 'MAINTENANCE')
        return JobCategoryType.MAINTENANCE;
    if (c === 'OPERATIONS')
        return JobCategoryType.OPERATIONS;
